...
testq = queue.Queue()
hbq = queue.Queue()
candev = CanSys.CanbusSystem.Config.candev
...
class CanbusControl:
        ...
        if not loopback:
            ...
            if do_can_up:
                ...
                if result > 0:
                    if result == 2:
                        self.logger.info(
                            f"{TerminalColors.Yellow}CAN Bus already started. Code=[{result}]{TerminalColors.RESET}")
                    elif result == 1:
                        self.logger.info(
    ...
    def stop(self):
        if self.CanbusHeartBeat is not None:
            self.CanbusHeartBeat.Deactivate()
            self.CanbusHeartBeat.join(timeout=10.0)
            if self.CanbusHeartBeat.is_alive():
                self.logger.info(
                    f"{TerminalColors.Red}Failed to terminate CAN bus heartbeat thread!{TerminalColors.RESET}")
        ...
        if self.command_thread is not None:
            ...
            if self.command_thread.is_alive():
                self.logger.info(
                    f"{TerminalColors.Red}Failed to terminate CAN bus command thread!{TerminalColors.RESET}")
                ...
                if not self.heartbeat_skip.is_set():
                    if self.hbmsg is not None:
                        try:
                            ...
                            self.logger.info(
                                f"{TerminalColors.Red}Heartbeat thread->CAN Bus Error: {ex}.{TerminalColors.RESET}")
        else:
            ...
            while self.commands_active.is_set():
                ...
                if len(s) > 0:
                    ...
                    try:
                        ...
                        self.canbus.flush_tx_buffer()
                        self.logger.info(
                            f"{TerminalColors.Red}Command Thread->CAN Bus Error: {ex}.{TerminalColors.RESET}")
...
class CanbusEventNode(threading.Thread):
    ...
    def run(self):
        ...
        if self.canbus is not None:  # can bus communication is present and ready for messages
            self.logger.info(f"{TerminalColors.Yellow}CAN Bus is available.{TerminalColors.RESET}")
        ...
        else:  # no can bus object is available
            self.logger.info(f"{TerminalColors.Red}CAN Bus not available!{TerminalColors.RESET}")
            while self.events_active.is_set():
                try:
