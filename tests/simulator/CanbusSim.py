...
def main():
    print(f"Simulation version {the_version}")
    ...
    parser.add_argument('--IDLIST', required=False,
                        help="The list if message IDs to monitor 0x001,0x043,0x310.  Default = 0x000")
    ...
    parser.add_argument('--FILTER', required=False,
                        help="The filter and mask settings if required. If not specified all messages are received.")
            ...
            if send_thread.is_alive():
                ...
                try:
                    if (t2 - t1).total_seconds() > send_period:
                        t1 = t2
                        msg = can.Message(timestamp=datetime.datetime.timestamp(t2), arbitration_id=ID, data=DATA,
